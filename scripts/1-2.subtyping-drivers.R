...
### Load MAF.GISTIC file, set some fancy colors to get cute
### visualization later.
...
### cluster-association data that we load, pre-process and then use to
### split.

file = read.delim(clusters.file, sep = ";")
head(file)

### Select  just certain annotations, remove blank lines at the end of
### the file.
...
### Filter out non-sequenced samples, and order them (for console
### visualization).
...
### TRONCO provides two functions to select a subset of samples. The
### one we use here subsets a TRONCO dataset according to a vector of
### names of samples. Once we subset we also trim as some of the
### events might have 0 frequency in our sub-cohort.
###
### The other function which splits a dataset according to a map is
### ssplit(). In the splitting process we get some warning as some
### samples for which we have clinical annotation are not found in the
### dataset -- for example TCGA-A6-3810. This happens because had not
### both mutations and CNAs.
...
### Now we start doing some fancier oncoprint visualization. We first
### set some colors also to annotate the pathways.

alteration.color = 'dimgray'
pathways.color = c('firebrick1', 'darkblue', 'darkgreen',
...
### Oncoprint has a lot of options, you might want to try these
...
### It can group genes with multiple alterations so to easily allow to
### spot which genes do not have, for instance, a CNA associated.
...
### It can highlight if some events - i.e. rows - have the same
### 'signature' namely occurr in the the same set of samples.
...
##           gene.annot.color = pathways.color,
##           sample.id = T,
##           annotate.consolidate.events = T)


### It can group samples according to a map, which helps to visualize
### clustering results.
...
oncoprint(as.pathway(MSS,
                     pathway.genes = Wnt,
                     pathway.name = 'Wnt',
                     aggregate.pathway = FALSE))

#### end of file -- 1-2.subtyping-drivers.R
