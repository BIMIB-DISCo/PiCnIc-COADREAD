...
###   - non-parametric: we re-sample a dataset of equale size of the
###     one used for model reconstruction, we re-run CAPRI, and then
###     count how many times we re-infer the same edge as compared to
###     how many times for each edge;
###   - statistical: we hold original data fixed, and we re-run the
###     statistical test for temporal priority and probability raising
###     by initializing a random number generator with different
###     seeds.
### Results of these procedures are matrices which report the score
### for each pair of edges in the model.

bootstrap.stat.conf = function(model, folder, ...) {
    ## This is set to 100 for our paper, here we set a much lower
    ## value as computation time might span over many hours with
    ## NUM.BOOT.ITER = 100
    ...
    ## As takes long to bootstrap, we make a simple visualization
    ## first.
    ...
    ## We also print to file the bootstrap scores -- we use the
    ## pheatmap package.
    
    if (DOPLOTS)
        pheatmap(keysToNames(model,
                             as.confidence(model,
    ...
    if (DOPLOTS)
        pheatmap(keysToNames(model,
                             as.confidence(model, conf = 'npb')$npb$capri_aic) * 100, 
    ...
    if (DOPLOTS)
        pheatmap(keysToNames(model,
                             as.confidence(model, conf = 'sb')$sb$capri_bic) * 100, 
    ...
    if (DOPLOTS)
        pheatmap(keysToNames(model,
                             as.confidence(model, conf = 'sb')$sb$capri_aic) * 100, 
...
### We print to screen a table with the bootstrap scores that we just
### computed.
### This function has pretty much the same options of
...
###   - eloss: entropy loss for each model computed from 10
...
if (DOPLOTS && FALSE) {
    vioplot(MSS.models$kfold$capri_bic$eloss,
            MSS.models$kfold$aic$eloss,
            col = 'red',
            lty = 1, rectCol="gray",
            colMed = 'black',
            names = c('BIC', 'AIC'), pchMed = 15, horizontal = T)
    title(main = 'Entropy loss \n MSS COADREAD tumors')
    ## dev.copy2pdf(file = 'MSS/MSS-kfold-eloss.pdf')

    vioplot(MSI.models$kfold$capri_bic$eloss,
            MSI.models$kfold$capri_aic$eloss,
            col = 'red',
            lty = 1,
            rectCol="gray",
            colMed = 'black',
...
### For instance, one can visualize a table with all edge statistics
### by merging all the table that we have produced with these
### functions.

tabular = function(obj, M) {
    tab = Reduce(function(...) merge(..., all = T), 
                 list(as.selective.advantage.relations(obj)[[M]],
...
addDataFrame(x = tabular(MSS.models, 'capri_bic'),
             sheet = sheet.mss.bic,
             showNA = T,
             characterNA = 'NA')
addDataFrame(x = tabular(MSS.models, 'capri_aic'),
             sheet = sheet.mss.aic,
             showNA = T,
             characterNA = 'NA')
addDataFrame(x = tabular(MSI.models, 'capri_bic'),
             sheet = sheet.msi.bic,
             showNA = T,
             characterNA = 'NA')
addDataFrame(x = tabular(MSI.models, 'capri_aic'),
             sheet = sheet.msi.aic,
             showNA = T,
             characterNA = 'NA')
